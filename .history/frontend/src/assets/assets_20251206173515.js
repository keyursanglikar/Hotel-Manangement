...
const API_BASE_URL = isProduction 
  ? (import.meta.env.VITE_PRODUCTION_API_BASE_URL || "https://hotel-manangement.onrender.com")
...
export const getImageURL = (imagePath) => {
  ...
  // If it's a backend uploaded image (contains 'uploads' or has a path)
  if (pathStr.includes('uploads') || pathStr.includes('/') && !pathStr.startsWith('/images/')) {
    const filename = pathStr.split('/').pop();
    return `${cleanApiUrl}/uploads/${filename}`;
  }
...
export const getFoodImage = (imageKey) => {
  ...
  // If it's an asset key (like 'food_1')
  if (typeof imageKey === 'string' && assets[imageKey]) {
    return assets[imageKey];
  }
