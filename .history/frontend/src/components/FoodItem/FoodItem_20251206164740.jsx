...
import { getFoodImage } from "../../assets/assets";
import "./FoodItem.css";
import { StoreContext } from '../context/StoreContext';
...
const FoodItem = ({ id, name, price, description, image, category }) => {
  ...
  // Get image URL
  const imageUrl = imgError ? "/images/placeholder.jpg" : getFoodImage(image);
    ...
    <div className="food-item">
      ...
      <div className="food-item-info">
        <div className="food-item-name-rating">
          <p>{name}</p>
          <div className="star">
            <img src="/images/rating_starts.png" alt="Rating" /> 
          </div>
        </div>
        <p className="food-item-desc">{description}</p>
        <p className="food-item-price">₹{Array.isArray(price) ? price.join(' ') : price}</p>
        {category && (
          <div className="food-item-category">
            <span className="category-tag">{category}</span>
          </div>
