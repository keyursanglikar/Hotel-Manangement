...
const StoreContextProvider = (props) => {
  ...
  const getBackendUrl = () => {
    if (import.meta.env.PROD) {
      return import.meta.env.VITE_PRODUCTION_API_BASE_URL || 
  ...
  const getTotalCartAmount = () => {
    ...
    for (const item in cartItems) {
      if (cartItems[item] > 0) {
        ...
        if (!itemInfo) {
          itemInfo = combinedFoodList.find((product) => product._id === item);
        }
        
        if (itemInfo) {
          const price = Array.isArray(itemInfo.price) ? 
            parseInt(itemInfo.price[0]) : 
            itemInfo.price;
  ...
  const fetchFoodList = async () => {
    try {
      ...
      const staticItems = food_list || [];
      setCombinedFoodList(staticItems);
      setAvailableCategories([...new Set(staticItems.map(item => item.category).filter(Boolean))].sort());
      return staticItems;
    }
  };
