...
import { createContext, useEffect, useState } from "react";
import axios from "axios";
import { food_list as staticFoodList, assets } from "../../assets/assets";
...
const StoreContextProvider = (props) => {
  ...
  // Function to normalize image format
  const normalizeImage = (item, isBackend = false) => {
    const normalizedItem = { ...item };
    
    if (isBackend) {
      // Backend item - ensure image is a string URL
      if (normalizedItem.image && typeof normalizedItem.image === 'string') {
        if (!normalizedItem.image.startsWith('http') && !normalizedItem.image.includes('uploads')) {
          // Add uploads path if missing
          normalizedItem.image = `uploads/${normalizedItem.image}`;
        }
      }
    } else {
      // Static item - ensure image is the actual imported image
      if (normalizedItem.image && typeof normalizedItem.image === 'string') {
        // Check if it's an asset key
        if (assets[normalizedItem.image]) {
          normalizedItem.image = assets[normalizedItem.image];
        }
      }
    }
  ...
  const getTotalCartAmount = () => {
    ...
    for (const item in cartItems) {
      if (cartItems[item] > 0) {
        ...
        if (!itemInfo) {
          itemInfo = staticFoodList.find((product) => product._id === item);
        }
  ...
  const fetchFoodList = async () => {
    try {
      ...
      // Process backend foods
      const backendItems = (response.data.data || []).map(item => 
        normalizeImage(item, true)
      );
      
      setFoodList(backendItems);
      
      // Process static foods
      const staticItems = (staticFoodList || []).map(item => 
        normalizeImage(item, false)
      );
      
      // Combine without duplicates - prefer backend items
      const combinedList = [...backendItems];
      
      // Add static items that don't exist in backend
      staticItems.forEach(staticItem => {
        const exists = backendItems.some(backendItem => 
          backendItem.name.toLowerCase() === staticItem.name.toLowerCase() ||
          backendItem._id === staticItem._id
        );
        if (!exists) {
          combinedList.push(staticItem);
        }
      });
      ...
      console.log("Combined list image types:");
      combinedList.slice(0, 3).forEach((item, index) => {
        console.log(`${index + 1}. ${item.name}:`, {
          hasImage: !!item.image,
          type: typeof item.image,
          value: item.image?.toString().substring(0, 50)
        });
      });
  ...
  const contextValue = {
    ...
    availableCategories,
    backend_food_list: foodList,
    static_food_list: staticFoodList,
    cartItems,
    setCartItems,
    addToCart,
    removeFromCart,
