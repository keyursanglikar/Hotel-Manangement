...
const StoreContextProvider = (props) => {
  ...
  // Function to process backend images
  const processBackendFoods = (backendFoods) => {
    return backendFoods.map(food => ({
      ...food,
  ...
  const fetchFoodList = async () => {
    try {
      ...
      const processedBackendFoods = processBackendFoods(response.data.data || []);
      setFoodList(processedBackendFoods);
      
      const backendItems = processedBackendFoods;
      const staticItems = food_list || [];
      
      // Remove duplicates - prefer backend items
      const uniqueStaticItems = staticItems.filter(staticItem => 
        !backendItems.some(backendItem => 
          backendItem.name.toLowerCase() === staticItem.name.toLowerCase()
        )
      ...
      // Combine both lists, backend first
      const combinedList = [...backendItems, ...uniqueStaticItems];
      setCombinedFoodList(combinedList);
      
      // Log image types for debugging
      console.log("Image types in combined list:");
      combinedList.slice(0, 3).forEach((item, index) => {
        console.log(`${index + 1}. ${item.name}:`, {
          image: item.image,
          type: typeof item.image,
          isString: typeof item.image === 'string',
          hasUploads: typeof item.image === 'string' && item.image.includes('uploads')
        });
      });
